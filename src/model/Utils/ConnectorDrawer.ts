...
export class ConnectorDrawer {
  ...
  drawConnectorBezierCurve(ctx : CanvasRenderingContext2D, 
    x1 : number, y1 : number, x2 : number, y2 : number, viewPort: BluePrintEditorViewport,
    ...
    if(pointPrecent >= 0) {
      ...
      dots.forEach((dotv) => {
        const of = MathUtils.threeOrderBezier(dotv, pos[0], pos[1], pos[2], pos[3], pos[4], pos[5], pos[6], pos[7]);
